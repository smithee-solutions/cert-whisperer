...
char
  param_CA [1024];
char
  param_CA_template [1024];
char
  param_certname [1024];
char
  param_config_file [1024];
char
  param_days [1024];
char
  param_email [1024];
char
  param_extensions [1024];
char
  param_subject [1024];
char
  param_config_file [1024];
...
{ /* initialize */
  ...
  while (!done)
  {
    ...
    default:
      ...
      fprintf (stderr, "  --subject=/C=US/ST=DC/L=Washington/O=org/OU=dept/CN=thing\n");
      break;
    case CW_CERTNAME:
      found_something = 1;
...
{ /* cert-herder */
  ...
  strcpy (param_subject, "/C=US/ST=California/L=Oakland/O=Example Organization/OU=LCBO/CN=User1 Operations1");
